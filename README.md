...
* Planos Grátis e VIP;
* **Backend de autenticação** (login, cadastro e recuperação de senha na Shiver via Playwright);
...
```text
ShiverLatan/
├── landing-page/    Next.js — landing, sala, sinais, SEO
├── backend/         Node.js + Express + Playwright — auth Shiver
└── documents/       tese, análises e documentação interna
```
...
* **Node.js 20+**
* **Express**
* **Playwright** (automação no site da Shiver)
* **dotenv**, **cors**
...
Detalhes da API e automação: [backend/README.md](backend/README.md).
...
### Landing (`landing-page/.env`)

```env
NEXT_PUBLIC_SITE_URL=https://seudominio.com
NEXT_PUBLIC_REGISTER_URL=https://auth.seudominio.com/login
```

`NEXT_PUBLIC_SITE_URL` deve ser informada **sem barra no final**.

#### Correto

```env
NEXT_PUBLIC_SITE_URL=https://shiverbroker.com
NEXT_PUBLIC_REGISTER_URL=https://auth.shiverbroker.com/login
```
...
`NEXT_PUBLIC_SITE_URL` é usada em sitemap, Open Graph, metadados e compartilhamento social.

`NEXT_PUBLIC_REGISTER_URL` define para onde o CTA **Conheça a ferramenta** aponta. Em desenvolvimento, o padrão é `http://localhost:3001/login`.
...
Em desenvolvimento local, use `HEADLESS=false` para acompanhar a automação no navegador.
...
que direciona para a **tela de login do backend** (`NEXT_PUBLIC_REGISTER_URL`), com fluxo:

```text
Login → (opcional) Cadastro ou Esqueci a senha → Sala / traderoom
```

---

# Autenticação (backend — Fase 1)

O backend recebe os dados do usuário e usa **Playwright** para interagir com o site oficial da Shiver. Não há API da corretora nem banco de dados nesta fase.

### Telas

* **Login** — e-mail e senha → `POST /api/login`
* **Cadastro** — nome, sobrenome, e-mail, senha e telefone (mesmos campos de `https://trade.shiverbroker.com/pt/register`) → `POST /api/register`
* **Esqueci a senha** — e-mail → `POST /api/forgot-password`

### Modo simulação

Com **Simular (não enviar à Shiver)** marcado, a automação preenche os formulários na Shiver mas **não envia** — útil para testes sem criar contas reais.

### Segurança

A senha existe apenas durante a requisição. Não é logada, gravada em arquivo nem incluída em screenshots.

### Fora desta fase

Ainda não implementado: iframe pós-login automático, cookies compartilhados com o navegador do usuário, sincronização de sessão, Google Login, fila e banco.
...
A sala e as telas de autenticação utilizam principalmente:
...
```text
          ...
          ┌────────────────┼────────────────┐
          │                │                │
       LANDING          BACKEND           /SALA
          │           (auth Playwright)      │
          │                │          ┌─────┴─────┐
   apresenta ferramenta    │          │           │
          │           login/cadastro  Assistente  Traderoom
          │           na corretora    │           │
...
* Privacidade;
* SEO base;
* **Backend de autenticação (Fase 1)** — login, cadastro e recuperação de senha via Playwright;
* **Telas de auth** alinhadas ao site da Shiver (`/login`, `/cadastro`, `/esqueci-senha`).

### Em evolução

* Integração definitiva do assistente;
* Sincronização de sessão entre backend e sala (iframe);
* Métricas;
