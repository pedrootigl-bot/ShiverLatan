...
import { MOCK_PAIR, MOCK_PRICE, MOCK_TOOL_METRICS } from "@/lib/mock-market-data";
    ...
    <section id="ferramenta" data-slide="3" className="deck-slide deck-slide--tool">
      <div className="deck-slide__layout">
        <div className="deck-slide__copy">
          <p className="deck-eyebrow deck-enter">{t.tool.eyebrow}</p>
          <h2 className="deck-title deck-title--compact">
            <span className="sr-only">{t.tool.sr} </span>
            <span className="deck-title__fill">{t.tool.fill}</span>
            <span className="deck-title__outline">{t.tool.outline}</span>
          </h2>
        </div>
        ...
        <div className="deck-visual deck-enter">
          <div className="deck-panel p-5 md:p-6">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              {MOCK_TOOL_METRICS.map((metric, index) => {
                const copy = t.tool.metrics[index];
                ...
                return (
                  ...
                  >
                    <p className="text-[10px] text-zinc-500">{copy?.label ?? metric.label}</p>
        ...
        <p className="deck-lead deck-enter">{t.tool.lead}</p>
      </div>
    </section>
  );
}
