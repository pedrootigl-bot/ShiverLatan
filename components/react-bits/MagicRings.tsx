...
type RingLiveProps = Omit<MagicRingsProps, "blur" | "className" | "active">;
...
const ctxMap = new WeakMap<HTMLDivElement, RingContext>();
...
export default function MagicRings({
  ...
  const propsRef = useRef<RingLiveProps | null>(null);
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      ...
      uniforms.uColor.value.set(p.color ?? "#38bdf8");
      uniforms.uColorTwo.value.set(p.colorTwo ?? "#2563eb");
    ...
    const tryStart = () => {
      if (!activeRef.current || !isVisible || !isPageVisible || frameId !== 0) {
        return;
      }
      lastT = 0;
      frameId = requestAnimationFrame(animate);
    ...
    ctxMap.set(mount, { tryStart, tryStop });
